...
SELECT multiIf(run_start = run_end, 'A lone-instant run',
                has(resumes, run_end), 'B1 resume at run end',
                'B2 paused through run end') AS family,
       count() AS final_zero_length_drops, uniqExact(video_session_id) AS sessions
FROM folded WHERE f.2 = run_end GROUP BY family ORDER BY family;
